...
const firebaseConfig = {
  apiKey: "AIzaSyCd0LqsSBSLVeEISLb2poPrXfMjpgj5S1k",
  authDomain: "social-network-b57be.firebaseapp.com",
  projectId: "social-network-b57be",
  storageBucket: "social-network-b57be.appspot.com",
  messagingSenderId: "801666377718",
  appId: "1:801666377718:web:17094e55823cbf472f09d7"
};
...
function cadastro$1(emailCad, passwordCad) {
...
const google = "/assets/google.6994ed57.png";
const login = () => {
  const container = document.createElement("div");
  const template = `
  <h2 class="titLogin"> Fa\xE7a o login ou cadastre-se </h2>
  ...
  <p id="pCadLogin"> Ainda n\xE3o possui conta? <a id="linkCadLogin" href="/#cadastro">Cadastre-se </a></p>
  `;
...
const db = vh(app);
const addPost = async (conteudo, displayName, data) => {
  yf(_h(db, "posts"), {
    conteudo,
    displayName,
    data
...
function mostrarPosts(infoFeed, limparTela) {
  const q2 = Rl(_h(db, "posts"), Cl("data", "desc"));
  pf(q2, (querySnapshot) => {
    limparTela();
    querySnapshot.forEach((document2) => {
      infoFeed(document2.data(), document2.id);
    });
  });
}
function atualizarPost(id2, novoConteudo) {
  const postAtual = mh(db, "posts", id2);
  mf(postAtual, {
    conteudo: novoConteudo
  });
}
function deletarPost(id2) {
  gf(mh(db, "posts", id2));
}
const feed = () => {
  ...
  const templateFeed = `
    ...
    <h2 id="titFeed"> Registre suas mem\xF3rias favoritas</h2>

    <form class="formFeed">
     <label id="postFeed" for="inputFeed"> escreva sua mensagem ou sua mem\xF3ria favorita:</label>
  ...
  function infoFeed(posts, id2) {
    const postContainer = document.createElement("div");
    const postTemplate = `
    <div id="postCompleto">
    <p id="nomeUser">${posts.displayName} </p>
    <textarea id="contPost${id2}" class="no-edit" disabled>${posts.conteudo} </textarea>
    <div id="boxPost">
    <button id="editarPost" data-editarid=${id2} >\u270F\uFE0F</button>
    <button id="salvarPost" data-salvarid=${id2} >\u2705</button>
    <button id="apagarPost" data-apagarid=${id2} >\u{1F5D1}\uFE0F</button>
    </div>
    </div>
    `;
  ...
  botaoFeed.addEventListener("click", (event) => {
    ...
    if (conteudoPost.value !== "") {
      addPost(conteudoPost.value, auth.currentUser.displayName, new Date()).then(() => {
      }).catch((error) => error);
    } else {
      const mensagem = containerFeed.querySelector("#mensagem");
      mensagem.innerHTML = "O post n\xE3o deve ser enviado vazio";
    }
  });
...
const about = () => {
  ...
  const templateAbout = `
      <h2 id="titAbout"> Sobre n\xF3s</h2>
      <p id="pAbout"> muitas informa\xE7\xF5es</p>
      `;
...
const cadastro = () => {
  ...
  const templateCad = `
    ...
    <label id="labelNome" for="inputNome"> Nome de usu\xE1rio:</label>
  ...
  botaoPagCad.addEventListener("click", (event) => {
    event.preventDefault();
    cadastro$1(emailCad.value, passwordCad.value).then(() => {
      nomeAtual(nome.value);
      window.location.hash = "#login";
    }).catch(() => {
      const msgError = containerCad.querySelector("#msgError");
      msgError.innerHTML = "seu cadastro n\xE3o foi efetuado";
    });
  });
  return containerCad;
};
